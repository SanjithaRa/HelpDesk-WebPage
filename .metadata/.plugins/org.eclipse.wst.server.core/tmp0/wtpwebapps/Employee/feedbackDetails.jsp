    ...
    <table border="1" class="styled-table">
        ...
        <c:forEach var="feedback" items="${feedbackDetails}">
            <tr>
                ...
                <td>${feedback.feedback}</td> <!-- Change Feedback to feedback -->
                
                <td>
                    <c:url value="updateFeedback.jsp" var="feedback_update">
                        <c:param name="id" value="${feedback.id}"/>
                        <c:param name="name" value="${feedback.name}"/>
                        <c:param name="date" value="${feedback.date}"/>
                        <c:param name="email" value="${feedback.email}"/>
                        <c:param name="feedback" value="${feedback.feedback}"/>
                    </c:url>
                    <a href="${feedback_update}">
                        <input type="button" name="update"  value="Update Details" class="update-button">
                    </a>
                </td>
                <td>
                    <c:url value="feedbackdelete.jsp" var="feeddelete">
                       <c:param name="id" value="${feedback.id}"/>
                        <c:param name="name" value="${feedback.name}"/>
                        <c:param name="date" value="${feedback.date}"/>
                        <c:param name="email" value="${feedback.email}"/>
